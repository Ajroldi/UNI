...
% Cerchio Mondo
\draw[thick, fill=yellow!20] (-2.5,0) circle (2.3cm);
\node at (-2.5,1.8) {\textbf{MONDO}};
\node[align=center] at (-2.5,0.3) {Ambiente\\Reale};

% Cerchio Macchina  
\draw[thick, fill=blue!20] (2.5,0) circle (2.3cm);
\node at (2.5,1.8) {\textbf{MACCHINA}};
\node[align=center] at (2.5,0.3) {Software\\Sistema};

% Intersezione
\begin{scope}
\clip (-2.5,0) circle (2.3cm);
\fill[green!30] (2.5,0) circle (2.3cm);
\end{scope}

\node[align=center] at (0,0.5) {\textbf{Fenomeni}};
\node[align=center] at (0,-0.2) {\textbf{Condivisi}};
\node[align=center, font=\small] at (0,-0.8) {(Interfaccia)};
\end{tikzpicture}
\end{center}
...
\node (req) [rectangle, draw, fill=green!20, text width=3.5cm, text centered, below left=of goal, minimum height=1.2cm] {\textbf{REQUIREMENTS}\\Software};
...
\draw[->, very thick] (req) -- node[left] {$\land$} (goal);
\draw[->, very thick] (dom) -- node[right] {$\land$} (goal);
...
% Sistema
\draw[thick] (3,0.5) rectangle (8,4.5);
...
\draw (6.5,3.5) ellipse (1.2cm and 0.5cm);
\node at (6.5,3.5) {Registrazione};

\draw (4.5,2) ellipse (1.5cm and 0.5cm);
\node at (4.5,2) {Invia Domanda};

\draw (6.5,2) ellipse (1.5cm and 0.5cm);
\node at (6.5,2) {Controlla Stato};

% Relazioni
\draw[->] (0.5,2.5) -- (3.3,3.5);
\draw[->] (0.5,2.5) -- (5.3,3.5);
\draw[->] (0.5,2.5) -- (3,2);
\draw[->] (0.5,2.5) -- (5,2);
\end{tikzpicture}
\end{center}
...
\begin{exbox}
\textbf{Esempio Completo: Prelevare Contante da ATM}

\textbf{Nome}: Prelevare Contante

\textbf{Attori}:
\begin{itemize}[leftmargin=*]
\item Primario: Cliente (bancario)
\item Secondario: Sistema Bancario (verifica saldo)
\end{itemize}

\textbf{Precondizioni}:
\begin{itemize}[leftmargin=*]
\item Cliente possiede carta ATM valida
\item ATM funzionante e con contante disponibile
\end{itemize}

\textbf{Flusso Principale}:
\begin{enumerate}[leftmargin=*]
\item Cliente inserisce carta ATM
\item Sistema legge carta e richiede PIN
\item Cliente inserisce PIN
\item Sistema verifica PIN con banca
\item Sistema mostra menu operazioni
\item Cliente seleziona ``Prelievo''
\item Cliente inserisce importo (es: 100€)
\item Sistema verifica saldo con banca
\item Sistema eroga contante
\item Sistema stampa ricevuta
\item Sistema restituisce carta
\item Cliente ritira contante e carta
\end{enumerate}

\textbf{Postcondizioni}:
\begin{itemize}[leftmargin=*]
\item Conto cliente addebitato dell'importo
\item Transazione registrata nel sistema
\item ATM aggiorna contante residuo
\end{itemize}

\textbf{Flussi Alternativi}:
\begin{itemize}[leftmargin=*]
\item \textbf{3a}: PIN errato → Sistema chiede reinserimento (max 3 tentativi)
\item \textbf{3b}: Dopo 3 tentativi errati → Sistema trattiene carta
\item \textbf{8a}: Saldo insufficiente → Sistema notifica e torna a passo 5
\item \textbf{9a}: ATM senza contante → Sistema mostra errore e termina
\end{itemize}

\textbf{Requisiti Speciali}:
\begin{itemize}[leftmargin=*]
\item Verifica PIN in < 3 secondi
\item Comunicazione con banca criptata (TLS 1.3)
\item Log transazione per audit
\end{itemize}
\end{exbox}
...
% Include example
\draw (0,3) ellipse (1.3cm and 0.5cm);
\node at (0,3) {Checkout};
\draw (3,3) ellipse (1.5cm and 0.5cm);
\node at (3,3) {Calcola Totale};
\draw[->, dashed, thick] (1.3,3) -- node[above] {«include»} (1.5,3);

% Extend example
\draw (0,1) ellipse (1cm and 0.5cm);
\node at (0,1) {Login};
\draw (3,1) ellipse (1.5cm and 0.5cm);
\node at (3,1) {Reset Password};
\draw[->, dashed, thick] (3,1) -- node[above] {«extend»} (1.5,1);

% Generalization example
\draw (0,-1) ellipse (1cm and 0.5cm);
\node at (0,-1) {Pagare};
\draw (2.5,-2) ellipse (1.4cm and 0.5cm);
\node at (2.5,-2) {Paga Carta};
\draw (2.5,0) ellipse (1.4cm and 0.5cm);
\node at (2.5,0) {Paga PayPal};
\draw[->, thick] (1.1,-1.8) -- (1,-1);
\draw[->, thick] (1.1,-0.2) -- (1,-1);
\end{tikzpicture}
\end{center}
...
\draw (0,2) -- (3,2);
\draw (0,1.2) -- (3,1.2);
\node at (1.5,2.25) {\textbf{Person}};
\node[align=left] at (1.5,1.6) {-name: String\\-age: int};
\node[align=left] at (1.5,0.6) {+getName()\\+setAge()};

% Classe Studente
\draw (5,0) rectangle (8,2.5);
\draw (5,2) -- (8,2);
\draw (5,1.2) -- (8,1.2);
\node at (6.5,2.25) {\textbf{Student}};
\node[align=left] at (6.5,1.6) {-studentID: String\\-course: String};
\node[align=left] at (6.5,0.6) {+enroll()\\+graduate()};

% Ereditarietà
\draw[->, thick] (5,1.25) -- (3,1.25);
\node[above] at (4,1.25) {\small extends};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
% Oggetti
\node[draw, rectangle] (user) at (0,4) {User};
\node[draw, rectangle] (system) at (3,4) {System};
\node[draw, rectangle] (db) at (6,4) {Database};

% Lifelines
\draw[dashed] (user) -- (0,0);
\draw[dashed] (system) -- (3,0);
\draw[dashed] (db) -- (6,0);

% Messaggi
\draw[->, thick] (0,3.5) -- node[above] {login()} (3,3.5);
\draw[->, thick] (3,3) -- node[above] {query()} (6,3);
\draw[->, thick, dashed] (6,2.5) -- node[above] {result} (3,2.5);
\draw[->, thick, dashed] (3,2) -- node[above] {success} (0,2);

% Activation boxes
\draw[fill=blue!20] (2.9,3.5) rectangle (3.1,2);
\draw[fill=green!20] (5.9,3) rectangle (6.1,2.5);
\end{tikzpicture}
\end{center}
...
\node[state, accepting, below of=active] (done) {Done};

\path[->] 
(idle) edge node {start} (active)
(active) edge [bend left] node {pause} (suspended)
(suspended) edge [bend left] node {resume} (active)
(active) edge node {finish} (done);
\end{tikzpicture}
\end{center}
